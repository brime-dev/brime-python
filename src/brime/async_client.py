...
from brime._response import AsyncAPIResponse
from brime._streaming import AsyncStream
...
class AsyncBrime:
    ...
    @cached_property
    def with_raw_response(self) -> _AsyncBrimeRaw:
        """Async variant of `Brime.with_raw_response`."""
        return _AsyncBrimeRaw(self)
    ...
    async def extract(
        ...
        *,
        ...
        data = await self._post_json("/v1/extract", body, idempotency_key=idem, timeout=timeout)
    ...
    async def research(
        ...
        *,
        ...
        data = await self._post_json("/v1/research", body, idempotency_key=idem, timeout=timeout)
        if depth == "basic":
    ...
    async def research_stream(
        ...
        *,
        ...
        timeout: float | None = None,
    ) -> AsyncStream[ResearchSseEvent]:
        """Stream research events asynchronously.

        Returns an `AsyncStream[ResearchSseEvent]` — use `async with` for
        guaranteed cleanup, or iterate directly with `async for`.
        """
        if depth == "basic":
            return await self._open_sse_post(
                "/v1/research",
                body={
                    "query": query,
                    "depth": "basic",
        ...
        return await self._open_sse_get(
    ...
    async def _post_json(
        ...
        *,
        ...
        res, attempts = await self.do_request_with_retry(
            "POST",
            path,
            body=body,
    ...
    async def _get_json(self, path: str, *, timeout: float | None = None) -> Any:
        res, attempts = await self.do_request_with_retry(
            "GET",
            path,
            body=None,
    ...
    async def do_request_with_retry(
        self,
        method: str,
        path: str,
        *,
        ...
        """Async variant of `Brime.do_request_with_retry` — same semantics."""
    ...
    async def _open_stream(self, request: httpx.Request) -> AsyncStream[ResearchSseEvent]:
        try:
...
class _AsyncBrimeRaw:
    """`AsyncBrime.with_raw_response` proxy — methods return `AsyncAPIResponse[T]`."""

    def __init__(self, client: AsyncBrime) -> None:
        self._client = client
    ...
    async def search(
        ...
        *,
        ...
        exclude_domains: list[str] | None = None,
        timeout: float | None = None,
    ) -> AsyncAPIResponse[SearchResponse]:
        body = _build_search_body(
            query=query,
            depth=depth,
            topic=topic,
        ...
        res, attempts = await self._client.do_request_with_retry(
            "POST",
            "/v1/search",
            body=body,
            idempotency_key=None,
        ...
        return AsyncAPIResponse(res, data, model_cls=SearchResponse, retries_taken=attempts)
    ...
    async def extract(
        ...
        *,
        ...
        idempotency_key: str | None = None,
        timeout: float | None = None,
    ) -> AsyncAPIResponse[ExtractResponse]:
        url_list = [urls] if isinstance(urls, str) else list(urls)
        body: dict[str, Any] = {
            "urls": url_list,
            "include_metadata": include_metadata,
        ...
        data = decode_response(res, retries_taken=attempts)
        return AsyncAPIResponse(res, data, model_cls=ExtractResponse, retries_taken=attempts)

    async def research_status(self, job_id: str) -> AsyncAPIResponse[ResearchStatusResponse]:
        res, attempts = await self._client.do_request_with_retry(
            "GET",
        ...
        data = decode_response(res, retries_taken=attempts)
        return AsyncAPIResponse(res, data, model_cls=ResearchStatusResponse, retries_taken=attempts)
